...
public final class ClientTimeController {
    ...
    public static void tick(Minecraft client) {
        ...
        if (config.mode() == DaylightDynamicsConfig.Mode.TIMEZONE) {
            ...
            world.getLevelData().setDayTime(-(dayBase + target));
            return;
        }
        ...
        if (ClientState.lastUpdateNanos() == 0L) {
            ...
            world.getLevelData().setDayTime(-world.getDayTime());
            return;
        }
        ...
        if (wholeTicks > 0L) {
            world.getLevelData().setDayTime(-(Math.abs(world.getDayTime()) + wholeTicks));
            accumulator -= wholeTicks;
        } else {
            world.getLevelData().setDayTime(-Math.abs(world.getDayTime()));
        }
